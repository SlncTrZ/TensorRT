...
Declarative Custom Kernels via ``torch_tensorrt.kernels.cuda_kernel_op``
==================================================================================

``cuda_kernel_op`` is the recommended entry point for exposing a
...
This example walks through the cases the declarative DSL covers:
...
For shape-changing kernels or anything outside this envelope, drop down to
:func:`torch_tensorrt.kernels.custom_cuda_kernel_op` (see
``custom_cuda_kernel_op.py``).
"""
...
def _run(name, model, inputs, ref, *, compile_trt=True):
    ...
    print(
        f"[{name}] eager matches reference:",
    ...
    with torch.no_grad():
        for _ in range(5):
            ...
            assert torch.allclose(out, ref, atol=1e-2, rtol=1e-2), f"{name}: mismatch"
